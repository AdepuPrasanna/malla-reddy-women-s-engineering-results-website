  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-2 text-muted">Full side-by-side marksheets with academic comparison</p>
      </header>
      ...
      <Card>
        <form onSubmit={handleCompare} className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <Button type="submit" loading={isFetching && !data} className="sm:w-auto">Compare Results</Button>
          </div>
        </form>
      </Card>
      ...
      {data && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          ...
          <section className="space-y-4">
            <h2 className="font-display text-xl font-semibold">Comparison Summary</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[data.first, data.second].map((r) => (
                <Card key={r.hallTicket}>
                  <h3 className="font-display font-bold">{r.studentName || "Student"}</h3>
                  <p className="text-sm text-primary-light">{r.hallTicket}</p>
                  <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                    <div>
                      <div className="text-xs text-muted">CGPA</div>
                      <div className="font-display text-lg font-bold">{r.cgpa || "—"}</div>
                    </div>
                    <div>
                    ...
                    <div>
                      <div className="text-xs text-muted">Backlogs</div>
                      <div className="font-display text-lg font-bold">{r.backlogCount ?? "—"}</div>
                    </div>
                  </div>
                </Card>
              ))}
              {data.comparison.cgpaDifference != null && (
                <Card className="flex flex-col items-center justify-center text-center sm:col-span-2 lg:col-span-2">
                  <div className="text-xs uppercase tracking-wider text-muted">CGPA Difference</div>
                  <div
                    className={`mt-2 font-display text-3xl font-bold ${
                      data.comparison.cgpaDifference >= 0 ? "text-success" : "text-error"
                    }`}
                  >
                    {data.comparison.cgpaDifference > 0 ? "+" : ""}
                    {data.comparison.cgpaDifference}
                  </div>
                  <p className="mt-1 text-xs text-muted">
                    {data.first.hallTicket} vs {data.second.hallTicket}
                  </p>
                </Card>
              )}
            </div>
          ...
          <section className="space-y-6">
            <h2 className="font-display text-xl font-semibold">Full Results</h2>
            <div className="space-y-10">
              <div>
                <h3 className="mb-4 font-display text-lg font-semibold text-primary-light">
                  Student 1 · {data.first.studentName || data.first.hallTicket}
                </h3>
                <ResultView data={data.first} />
              </div>
              <div>
                <h3 className="mb-4 font-display text-lg font-semibold text-primary-light">
                  Student 2 · {data.second.studentName || data.second.hallTicket}
                </h3>
                <ResultView data={data.second} />
              </div>
            </div>
          </section>
        </motion.div>
